          ...
          orders.map((order) => (
            ...
            >
              ...
              <p className="text-xs text-gray-500 mb-1">
                Table {order.tableNumber} &bull; {formatDate(order.createdAt)}
              </p>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRowSkeleton key={i} cols={showActions ? 7 : 6} />
                ))
              ) : orders.length === 0 ? (
                <tr>
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className="flex items-center gap-2">
                        ...
                        {newOrderIds.has(order._id) && (
                          <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" title="New order" />
                        )}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 min-w-[150px] max-w-[280px]">
                      <span className="block break-words">{order.items.map((i) => `${i.name} x${i.quantity}`).join(', ')}</span>
                    </td>
