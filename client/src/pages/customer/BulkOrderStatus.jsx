...
export default function BulkOrderStatus() {
  ...
  useEffect(() => {
    ...
    const handler = ({ orderId, status, order: updatedOrder }) => {
      if (orderId === id) {
        setOrder((prev) =>
          prev ? { ...prev, status, ...(updatedOrder || {}) } : prev
        );
        toast.info(`Bulk order status: ${status}`);
      }
    };
  ...
  const stepIndex = order && order.status !== 'rejected'
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col pb-20 md:pb-0">
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-16 text-center">
          <p className="text-gray-500 text-lg">Bulk order not found</p>
          <Link
            to="/orders"
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 mb-5 sm:mb-6">
          <div className="flex items-center justify-between mb-3 sm:mb-4 gap-2">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">
              Bulk Order #{order._id.slice(-6).toUpperCase()}
            </h2>
            <OrderStatusBadge status={order.status} />
          </div>
          <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6">
            Pickup: {new Date(order.pickupDate).toLocaleDateString()}{' '}
            &bull; {new Date(order.createdAt).toLocaleString()}
          </p>
              ...
              {STEPS.map((step, i) => (
                <div key={step} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] sm:text-xs text-gray-500 mt-1.5 sm:mt-2 capitalize truncate max-w-full">
                      {step}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
